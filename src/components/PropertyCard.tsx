...
const PropertyCard = forwardRef<HTMLDivElement, PropertyCardProps>(
  ({ property, isSelected, isFavorite, onToggleFavorite, onClick, onExpand }, ref) => {
      ...
      >
        <div className="relative">
          ...
          {onToggleFavorite && (
            <button
              ...
              className={`absolute right-2.5 top-2.5 p-2 rounded-full transition-all duration-200 ${
                isFavorite
        ...
        <button onClick={onClick} className="w-full text-left">
          <div className="p-3.5 pt-3">
            <h3 className="font-semibold text-sm text-card-foreground leading-tight line-clamp-1">{property.title}</h3>
            <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
              <MapPin size={12} strokeWidth={SW} /> {property.neighborhood}
            </p>
            <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground flex-wrap">
              <span className="flex items-center gap-1">
                <Ruler size={12} strokeWidth={SW} /> {property.area}m²
              </span>
              {!isMidia && property.bedrooms != null && (
                <span className="flex items-center gap-1">
                  <BedDouble size={12} strokeWidth={SW} /> {property.bedrooms}
                </span>
              )}
              {!isMidia && property.bathrooms != null && (
                <span className="flex items-center gap-1">
                  <Bath size={12} strokeWidth={SW} /> {property.bathrooms}
                </span>
              )}
              {!isMidia && property.garageSpaces != null && (
                <span className="flex items-center gap-1">
                  <Car size={12} strokeWidth={SW} /> {property.garageSpaces}
                </span>
              )}
              {isMidia && property.mediaType && (
                <span className="flex items-center gap-1">
                  <Megaphone size={12} strokeWidth={SW} /> {mediaTypeLabels[property.mediaType]}
                </span>
              )}
            </div>
        ...
        <div className="px-3.5 pb-2">
          <button
            onClick={(e) => {
              e.stopPropagation();
              onExpand ? onExpand() : setExpanded(!expanded);
            }}
            className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1 transition-colors"
          >
            {!onExpand && expanded ? (
              <ChevronUp size={14} strokeWidth={SW} />
            ) : (
              <ChevronDown size={14} strokeWidth={SW} />
            )}
